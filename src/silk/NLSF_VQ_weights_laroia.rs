...
pub unsafe fn silk_NLSF_VQ_weights_laroia(pNLSFW_Q_OUT: *mut i16, pNLSF_Q15: *const i16, D: i32) {
    ...
    assert!(D > 0);
    assert!(D & 1 == 0);
    tmp1_int = silk_max_int(*pNLSF_Q15.offset(0 as isize) as i32, 1);
    tmp1_int = ((1) << 15 + 2) / tmp1_int;
    tmp2_int = silk_max_int(
        *pNLSF_Q15.offset(1 as isize) as i32 - *pNLSF_Q15.offset(0 as isize) as i32,
        1,
    );
    tmp2_int = ((1) << 15 + 2) / tmp2_int;
    *pNLSFW_Q_OUT.offset(0 as isize) = silk_min_int(tmp1_int + tmp2_int, silk_int16_MAX) as i16;
    k = 1;
    while k < D - 1 {
    ...
    tmp1_int = silk_max_int(((1) << 15) - *pNLSF_Q15.offset((D - 1) as isize) as i32, 1);
    tmp1_int = ((1) << 15 + 2) / tmp1_int;
    *pNLSFW_Q_OUT.offset((D - 1) as isize) =
        silk_min_int(tmp1_int + tmp2_int, silk_int16_MAX) as i16;
}
