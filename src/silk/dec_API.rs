...
pub mod main_h {
    ...
    extern "C" {
        ...
        pub fn silk_stereo_decode_pred(psRangeDec: *mut ec_dec, pred_Q13: *mut opus_int32);
...
pub unsafe extern "C" fn silk_InitDecoder(mut decState: *mut libc::c_void) -> libc::c_int {
    ...
    let mut ret: libc::c_int = 0 as libc::c_int;
    let mut channel_state: *mut silk_decoder_state =
...
pub unsafe extern "C" fn silk_Decode(
    ...
    if (*decControl).nChannelsInternal == 2 as libc::c_int {
                    ...
                    == 1 as libc::c_int
        {
            ...
            n = 0 as libc::c_int;
            while n < 2 as libc::c_int {
                MS_pred_Q13[n as usize] = (*psDec).sStereo.pred_prev_Q13[n as usize] as opus_int32;
                n += 1;
            }
        }
    }
        ...
        && (*decControl).nChannelsInternal == 2 as libc::c_int
    {
        ...
        memcpy(
            ...
            .offset(nSamplesOutDec as isize) as *mut opus_int16 as *const libc::c_void,
            (2 as libc::c_int as libc::c_ulong)
                .wrapping_mul(::core::mem::size_of::<opus_int16>() as libc::c_ulong),
        );
    }
    *nSamplesOut = nSamplesOutDec * (*decControl).API_sampleRate
        / ((*channel_state.offset(0 as libc::c_int as isize)).fs_kHz as opus_int16 as opus_int32
    ...
    let mut samplesOut1_tmp_storage2: Vec<opus_int16> = ::std::vec::from_elem(0, vla_1);
    if delay_stack_alloc != 0 {
        memcpy(
    ...
    if lostFlag == 1 as libc::c_int {
        ...
        while i < (*psDec).nChannelsInternal {
            (*psDec).channel_state[i as usize].LastGainIndex = 10 as libc::c_int as opus_int8;
            i += 1;
        }
    } else {
