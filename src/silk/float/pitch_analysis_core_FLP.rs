...
pub unsafe fn silk_pitch_analysis_core_FLP(
    ...
    sf_length_4kHz = PE_SUBFR_LENGTH_MS * 4 as i32;
    sf_length_8kHz = PE_SUBFR_LENGTH_MS * 8 as i32;
    min_lag = PE_MIN_LAG_MS * Fs_kHz;
    min_lag_4kHz = PE_MIN_LAG_MS * 4 as i32;
    min_lag_8kHz = PE_MIN_LAG_MS * 8 as i32;
    max_lag = PE_MAX_LAG_MS * Fs_kHz - 1 as i32;
    max_lag_4kHz = PE_MAX_LAG_MS * 4 as i32;
    max_lag_8kHz = PE_MAX_LAG_MS * 8 as i32 - 1 as i32;
    if Fs_kHz == 16 as i32 {
        ...
        memset(
            filt_state.as_mut_ptr() as *mut core::ffi::c_void,
            0 as i32,
            (6 as i32 as u64).wrapping_mul(::core::mem::size_of::<i32>() as u64),
        );
    ...
    while i > 0 as i32 {
        ...
        {
            silk_int16_MAX as f32
        } else if frame_4kHz[i as usize] as i32 as f32 + frame_4kHz[(i - 1 as i32) as usize]
            < silk_int16_MIN as f32
        {
            silk_int16_MIN as f32
        } else {
            frame_4kHz[i as usize] as i32 as f32 + frame_4kHz[(i - 1 as i32) as usize]
        }) as i16 as f32;
        i -= 1;
    }
    memset(
        ...
        (::core::mem::size_of::<f32>() as u64)
            ...
            .wrapping_mul(((18 as i32 * 16 as i32 >> 1 as i32) + 5 as i32) as u64),
    );
            ...
            normalizer += *basis_ptr.offset(0 as i32 as isize) as f64
                * *basis_ptr.offset(0 as i32 as isize) as f64
                - *basis_ptr.offset(sf_length_8kHz as isize) as f64
                    * *basis_ptr.offset(sf_length_8kHz as isize) as f64;
            C[0 as i32 as usize][d as usize] += (2 as i32 as f64 * cross_corr / normalizer) as f32;
            d += 1;
        }
    ...
    while i >= min_lag_4kHz {
        C[0 as i32 as usize][i as usize] -= C[0 as i32 as usize][i as usize] * i as f32 / 4096.0f32;
        i -= 1;
    }
    ...
    while i < length_d_srch {
        if C[0 as i32 as usize][(min_lag_4kHz + i) as usize] > threshold {
            d_srch[i as usize] = (((d_srch[i as usize] + min_lag_4kHz) as u32) << 1 as i32) as i32;
            i += 1;
        } else {
            length_d_srch = i;
            break;
    ...
    i = 0 as i32;
    while i < length_d_srch {
        d_comp[d_srch[i as usize] as usize] = 1 as i32 as i16;
        i += 1;
    }
    ...
    while i >= min_lag_8kHz {
        d_comp[i as usize] = (d_comp[i as usize] as i32
            + (d_comp[(i - 1 as i32) as usize] as i32
                ...
                + d_comp[(i - 3 as i32) as usize] as i32)) as i16;
        i -= 1;
    }
    ...
    while i < max_lag_8kHz + 4 as i32 {
        if d_comp[i as usize] as i32 > 0 as i32 {
            d_comp[length_d_comp as usize] = (i - 2 as i32) as i16;
            length_d_comp += 1;
        }
        i += 1;
    }
    memset(
        C.as_mut_ptr() as *mut core::ffi::c_void,
        0 as i32,
        ((4 as i32 * ((18 as i32 * 16 as i32 >> 1 as i32) + 5 as i32)) as u64)
            .wrapping_mul(::core::mem::size_of::<f32>() as u64),
    );
    if Fs_kHz == 8 as i32 {
        target_ptr = &*frame.offset((PE_LTP_MEM_LENGTH_MS * 8 as i32) as isize) as *const f32;
    } else {
        target_ptr = &mut *frame_8kHz
            .as_mut_ptr()
            .offset((PE_LTP_MEM_LENGTH_MS * 8 as i32) as isize) as *mut f32;
    }
    k = 0 as i32;
    while k < nb_subfr {
        ...
        while j < length_d_comp {
            ...
            if cross_corr > 0.0f32 as f64 {
                ...
                C[k as usize][d as usize] =
                    (2 as i32 as f64 * cross_corr / (energy + energy_tmp)) as f32;
            } else {
    ...
    if prevLag > 0 as i32 {
        if Fs_kHz == 12 as i32 {
            prevLag = ((prevLag as u32) << 1 as i32) as i32 / 3 as i32;
    ...
    if nb_subfr == PE_MAX_NB_SUBFR {
        ...
        if Fs_kHz == 8 as i32 && complexity > SILK_PE_MIN_COMPLEX {
    ...
    if Fs_kHz > 8 as i32 {
        if Fs_kHz == 12 as i32 {
            lag = if 1 as i32 == 1 as i32 {
                (lag as i16 as i32 * 3 as i32 as i16 as i32 >> 1 as i32)
                    + (lag as i16 as i32 * 3 as i32 as i16 as i32 & 1 as i32)
            } else {
                (lag as i16 as i32 * 3 as i32 as i16 as i32 >> 1 as i32 - 1 as i32) + 1 as i32
                    >> 1 as i32
            };
        } else {
            lag = ((lag as u32) << 1 as i32) as i32;
        }
        ...
        start_lag = silk_max_int(lag - 2 as i32, min_lag);
        ...
        while d <= end_lag {
            ...
            while j < nb_cbk_search {
                ...
                if cross_corr > 0.0f64 {
                    CCmax_new = (2 as i32 as f64 * cross_corr / energy) as f32;
                    CCmax_new *= 1.0f32 - contour_bias * j as f32;
                } else {
                    CCmax_new = 0.0f32;
                }
                if CCmax_new > CCmax
                    && d + silk_CB_lags_stage3[0 as i32 as usize][j as usize] as i32 <= max_lag
                {
                    CCmax = CCmax_new;
                    lag_new = d;
                    CBimax = j;
...
unsafe fn silk_P_Ana_calc_corr_st3(
    ...
    target_ptr = &*frame.offset(((sf_length as u32) << 2 as i32) as i32 as isize) as *const f32;
    k = 0 as i32;
    while k < nb_subfr {
        lag_counter = 0 as i32;
        lag_low = *Lag_range_ptr.offset((k * 2 as i32 + 0 as i32) as isize) as i32;
        lag_high = *Lag_range_ptr.offset((k * 2 as i32 + 1 as i32) as isize) as i32;
        celt_pitch_xcorr_c(
            target_ptr,
            target_ptr
        ...
        delta = *Lag_range_ptr.offset((k * 2 as i32 + 0 as i32) as isize) as i32;
        i = 0 as i32;
        while i < nb_cbk_search {
...
unsafe fn silk_P_Ana_calc_energy_st3(
    ...
    while k < nb_subfr {
        ...
        basis_ptr = target_ptr.offset(
            -((start_lag + *Lag_range_ptr.offset((k * 2 as i32 + 0 as i32) as isize) as i32)
                as isize),
        );
        energy = silk_energy_FLP(basis_ptr, sf_length) + 1e-3f64;
        scratch_mem[lag_counter as usize] = energy as f32;
        lag_counter += 1;
        lag_diff = *Lag_range_ptr.offset((k * 2 as i32 + 1 as i32) as isize) as i32
            - *Lag_range_ptr.offset((k * 2 as i32 + 0 as i32) as isize) as i32
            + 1 as i32;
        i = 1 as i32;
        while i < lag_diff {
