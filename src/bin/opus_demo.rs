...
unsafe fn main_0() -> i32 {
    ...
    let mut nrg: f64 = 0.;
    let mut tot_samples: f64 = 0 as f64;
    let mut tot_in: u64 = 0;
    let mut tot_out: u64 = 0;
    let mut bandwidth: i32 = OPUS_AUTO;
    let mut lost: i32 = 0;
    let mut lost_prev: i32 = 1;
    let mut toggle: i32 = 0;
    let mut enc_final_range: [u32; 2] = [0; 2];
    let mut dec_final_range: u32 = 0;
    let mut encode_only: i32 = 0;
    let mut decode_only: i32 = 0;
    let mut max_frame_size: i32 = 48000 * 2;
    let mut num_read: size_t = 0;
    let mut curr_read: i32 = 0;
    let mut sweep_bps: i32 = 0;
    let mut random_framesize: i32 = 0;
    let mut newsize: i32 = 0;
    let mut delayed_celt: i32 = 0;
    let mut sweep_max: i32 = 0;
    let mut sweep_min: i32 = 0;
    let mut random_fec: i32 = 0;
    let mut mode_list: *const [i32; 4] = std::ptr::null::<[i32; 4]>();
    let mut nb_modes_in_list: i32 = 0;
    let mut curr_mode: i32 = 0;
    let mut curr_mode_count: i32 = 0;
    let mut mode_switch_time: i32 = 48000;
    let mut nb_encoded: i32 = 0;
    let mut remaining: i32 = 0;
    let mut variable_duration: i32 = 5000;
    let mut delayed_decision: i32 = 0;
    let mut ret: i32 = 1;
    ...
    if max_payload_bytes < 0 || max_payload_bytes > MAX_PACKET as i32 {
    ...
    if decode_only == 0 {
        ...
        if err != 0 {
            let err = opus_strerror(err);
            eprintln!("Cannot create encoder: {}", err);
            return 1;
        } else {
            opus_encoder_ctl!(enc, 4002, bitrate_bps,);
            opus_encoder_ctl!(enc, 4008, bandwidth,);
            opus_encoder_ctl!(enc, 4006, use_vbr,);
            opus_encoder_ctl!(enc, 4020, cvbr,);
            opus_encoder_ctl!(enc, 4010, complexity,);
            opus_encoder_ctl!(enc, 4012, use_inbandfec,);
            opus_encoder_ctl!(enc, 4022, forcechannels,);
            opus_encoder_ctl!(enc, 4016, use_dtx,);
            opus_encoder_ctl!(enc, 4014, packet_loss_perc,);
            opus_encoder_ctl!(enc, 4027, &mut skip);
            opus_encoder_ctl!(enc, 4036, 16,);
            opus_encoder_ctl!(enc, 4040, variable_duration,);
        }
    }
    ...
    data[0 as usize] = calloc(max_payload_bytes as u64, 1) as *mut u8;
    if use_inbandfec != 0 {
        data[1 as usize] = calloc(max_payload_bytes as u64, 1) as *mut u8;
    }
    ...
    loop {
        ...
        if delayed_celt != 0 {
            frame_size = newsize;
            delayed_celt = 0;
        } else if random_framesize != 0 && rand() % 20 == 0 {
            newsize = match rand() % 6 {
                0 => sampling_rate / 400,
                1 => sampling_rate / 200,
                2 => sampling_rate / 100,
                3 => sampling_rate / 50,
                4 => sampling_rate / 25,
                5 => 3 * sampling_rate / 50,
                _ => unreachable!(),
            };
            ...
            if newsize < sampling_rate / 100 && frame_size >= sampling_rate / 100 {
                opus_encoder_ctl!(enc, 11002, 1002,);
                delayed_celt = 1;
            } else {
                frame_size = newsize;
            }
        }
        if random_fec != 0 && rand() % 30 == 0 {
            rand();
            opus_encoder_ctl!(enc, 4012, (rand() % 4 == 0) as i32,);
        }
        if decode_only != 0 {
            let mut ch: [u8; 4] = [0; 4];
            ...
            if len[toggle as usize] > max_payload_bytes || len[toggle as usize] < 0 {
            ...
            if !mode_list.is_null() {
                opus_encoder_ctl!(
                    enc,
                    4008,
                    (*mode_list.offset(curr_mode as isize))[1 as usize],
                );
                opus_encoder_ctl!(
                    enc,
                    11002,
                    (*mode_list.offset(curr_mode as isize))[0 as usize],
                );
                opus_encoder_ctl!(
                    enc,
                    4022,
                    (*mode_list.offset(curr_mode as isize))[3 as usize],
                );
                frame_size = (*mode_list.offset(curr_mode as isize))[2 as usize];
            }
            num_read = fin
            ...
            while i < curr_read * channels {
                let mut s: i32 = 0;
                s = (*fbytes.offset((2 * i + 1) as isize) as i32) << 8
                    | *fbytes.offset((2 * i) as isize) as i32;
        ...
        if encode_only != 0 {
            ...
            lost = (len[toggle as usize] == 0
                || packet_loss_perc > 0 && (rand() % 100) < packet_loss_perc)
                as i32;
            if lost != 0 {
                opus_decoder_ctl!(dec, 4039, &mut output_samples);
            } else {
            ...
            if count >= use_inbandfec {
                if use_inbandfec != 0 {
                    if lost_prev != 0 {
                        ...
                        output_samples = opus_decode(
                            dec,
                            data[(1 - toggle) as usize],
                            len[(1 - toggle) as usize],
                            out,
                            output_samples,
                            0,
                        );
                ...
                if output_samples > 0 {
                    ...
                    if output_samples > skip {
                        ...
                        while i_0 < (output_samples - skip) * channels {
                            ...
                            *fbytes.offset((2 * i_0) as isize) = (s_0 as i32 & 0xff) as u8;
                            *fbytes.offset((2 * i_0 + 1) as isize) = (s_0 as i32 >> 8 & 0xff) as u8;
                            i_0 += 1;
                        }
                        fout.write_all(std::slice::from_raw_parts(
                            fbytes,
                    ...
                    eprintln!("opus_decode() returned {}", opus_strerror(output_samples));
                }
        ...
        if enc_final_range[(toggle ^ use_inbandfec) as usize] != 0
            && encode_only == 0
            && lost == 0
            && lost_prev == 0
            && dec_final_range != enc_final_range[(toggle ^ use_inbandfec) as usize]
        {
            ...
            lost_prev = lost;
            if count >= use_inbandfec {
                bits += (len[toggle as usize] * 8) as f64;
                bits_max = if (len[toggle as usize] * 8) as f64 > bits_max {
                    (len[toggle as usize] * 8) as f64
                } else {
                    bits_max
                };
                bits2 += (len[toggle as usize] * len[toggle as usize] * 64) as f64;
                if decode_only == 0 {
                    nrg = 0.0f64;
                    k = 0;
                    ...
                    if nrg > 1e5f64 {
                        bits_act += (len[toggle as usize] * 8) as f64;
                        count_act += 1;
                    }
                }
            }
    ...
    if decode_only != 0 && count > 0 {
        frame_size = (tot_samples / count as f64) as i32;
    }
    count -= use_inbandfec;
    if tot_samples >= 1 as f64 && count > 0 && frame_size != 0 {
        let mut var: f64 = 0.;
        eprintln!(
    ...
    // free(data[0 as usize] as *mut core::ffi::c_void);
    // free(data[1 as usize] as *mut core::ffi::c_void);
    // if !fin.is_null() {
    //     fclose(fin);
    // }
    // if !fout.is_null() {
