...
pub unsafe extern "C" fn generate_music(mut buf: *mut libc::c_short, mut len: opus_int32) {
    let mut a1: opus_int32 = 0;
    let mut b1: opus_int32 = 0;
    let mut a2: opus_int32 = 0;
    let mut b2: opus_int32 = 0;
    let mut c1: opus_int32 = 0;
    let mut c2: opus_int32 = 0;
    let mut d1: opus_int32 = 0;
    let mut d2: opus_int32 = 0;
    ...
    while i < 2880 as libc::c_int {
        let ref mut fresh0 = *buf.offset((i * 2 as libc::c_int + 1 as libc::c_int) as isize);
    ...
    while i < len {
        ...
        let mut v1: opus_int32 = 0;
        let mut v2: opus_int32 = 0;
        v2 = ((j
            * (j >> 12 as libc::c_int
        ...
        v1 = v2;
        r = fast_rand();
        v1 = (v1 as libc::c_uint).wrapping_add(r & 65535 as libc::c_int as libc::c_uint)
            as opus_int32 as opus_int32;
        v1 = (v1 as libc::c_uint).wrapping_sub(r >> 16 as libc::c_int) as opus_int32 as opus_int32;
        r = fast_rand();
        v2 = (v2 as libc::c_uint).wrapping_add(r & 65535 as libc::c_int as libc::c_uint)
            as opus_int32 as opus_int32;
        v2 = (v2 as libc::c_uint).wrapping_sub(r >> 16 as libc::c_int) as opus_int32 as opus_int32;
...
pub unsafe extern "C" fn test_encode(
    ...
    let mut inbuf: *mut opus_int16 = 0 as *mut opus_int16;
    let mut packet: [libc::c_uchar; 1757] = [0; 1757];
    let mut len: libc::c_int = 0;
    let mut outbuf: *mut opus_int16 = 0 as *mut opus_int16;
    let mut out_samples: libc::c_int = 0;
    let mut ret: libc::c_int = 0 as libc::c_int;
    inbuf = malloc(
        (::core::mem::size_of::<opus_int16>() as libc::c_ulong).wrapping_mul(
...
pub unsafe extern "C" fn run_test1(mut no_fuzz: libc::c_int) -> libc::c_int {
    ...
    let mut err: libc::c_int = 0;
    let mut enc: *mut OpusEncoder = 0 as *mut OpusEncoder;
    let mut MSenc: *mut OpusMSEncoder = 0 as *mut OpusMSEncoder;
    let mut dec: *mut OpusDecoder = 0 as *mut OpusDecoder;
    let mut MSdec: *mut OpusMSDecoder = 0 as *mut OpusMSDecoder;
    let mut MSdec_err: *mut OpusMSDecoder = 0 as *mut OpusMSDecoder;
    let mut dec_err: [*mut OpusDecoder; 10] = [0 as *mut OpusDecoder; 10];
    let mut inbuf: *mut libc::c_short = 0 as *mut libc::c_short;
    let mut outbuf: *mut libc::c_short = 0 as *mut libc::c_short;
    let mut out2buf: *mut libc::c_short = 0 as *mut libc::c_short;
    let mut bitrate_bps: opus_int32 = 0;
    let mut packet: [libc::c_uchar; 1757] = [0; 1757];
    ...
    let mut tmp_enc: *mut OpusEncoder = 0 as *mut OpusEncoder;
    if opus_multistream_encoder_ctl(
        MSenc,
        5120 as libc::c_int,
        1 as libc::c_int,
    ...
    if opus_encoder_ctl(
        tmp_enc,
        4037 as libc::c_int,
        (&mut j as *mut opus_int32).offset(
            (&mut j as *mut opus_int32).offset_from(&mut j as *mut opus_int32) as libc::c_long
                as isize,
        ),
    ) != 0 as libc::c_int
    {
    ...
    let mut enccpy: *mut OpusEncoder = 0 as *mut OpusEncoder;
        ...
        while j < 13 as libc::c_int {
            ...
            loop {
                ...
                if opus_encoder_ctl(
                    enc,
                    4016 as libc::c_int,
                    (fast_rand() & 1 as libc::c_int as libc::c_uint) as opus_int32,
                ) != 0 as libc::c_int
                {
                    _test_failed(
                        b"tests/test_opus_encode.c\0" as *const u8 as *const libc::c_char,
                ...
                if opus_encoder_ctl(
                    ...
                    (&mut enc_final_range as *mut opus_uint32).offset(
                        (&mut enc_final_range as *mut opus_uint32)
                            .offset_from(&mut enc_final_range as *mut opus_uint32)
    ...
    while rc < 3 as libc::c_int {
        ...
        while j < 16 as libc::c_int {
            ...
            loop {
                ...
                let mut pred: opus_int32 = 0;
                if opus_multistream_encoder_ctl(
                    MSenc,
                    4043 as libc::c_int,
                    (&mut pred as *mut opus_int32).offset(
                        (&mut pred as *mut opus_int32).offset_from(&mut pred as *mut opus_int32)
                            as libc::c_long as isize,
                    ),
                ) != 0 as libc::c_int
                {
                    ...
                    == 0 as libc::c_int as libc::c_uint
                {
                    ...
                    if fast_rand() & 3 as libc::c_int as libc::c_uint
                        != 0 as libc::c_int as libc::c_uint
                    {
                        if opus_multistream_decoder_ctl(MSdec_err, 4028 as libc::c_int)
                            != 0 as libc::c_int
                        {
                            _test_failed(
                                b"tests/test_opus_encode.c\0" as *const u8 as *const libc::c_char,
    ...
    loop {
        ...
        let mut offset: opus_int32 =
            i % (48000 as libc::c_int * 30 as libc::c_int - 5760 as libc::c_int);
        ...
        opus_decoder_ctl(
            dec_err[dec2 as usize],
            4031 as libc::c_int,
            (&mut dec_final_range2 as *mut opus_uint32).offset(
                (&mut dec_final_range2 as *mut opus_uint32)
                    .offset_from(&mut dec_final_range2 as *mut opus_uint32)
                    as libc::c_long as isize,
            ),
        );
...
unsafe fn main_0(mut _argc: libc::c_int, mut _argv: *mut *mut libc::c_char) -> libc::c_int {
    ...
    let mut strtol_str: *mut libc::c_char = 0 as *mut libc::c_char;
